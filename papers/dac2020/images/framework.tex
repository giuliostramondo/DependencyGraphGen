...
%  For compatability with PGF CVS add the absolute option:
   ...
   \begin{scope}[xshift=-7.5cm,yshift=-5cm,very thick,
    ...
    \node [block, above=of dse,above=20 pt] (dfg_full) {Maximal Parallel Architecture};
    \node [block, below=of dse,below=20pt] (dfg_seq) {Maximal Sequential Architecture};
%    \node [circle,draw,fill=blue!100,right=of dse,text=white,font=\sffamily] (l22){2};
    \node [event,right=of dse] (l22){Model L2 write};

    \node [block, right of=l22] (dse_analysis) {Architectures Tradeoffs};
  \node [event,fill=green!20 , below of=dse_analysis, below = -50pt, text width=2.5cm] (impl) {Implementation};
...
%    \draw [->] (dse) -- (dse_analysis);
    ...
    \draw [->, dashed] (dse_analysis) -- (impl);
   \draw [->, dashed] (temp_arch) -- (impl);
 \draw [->, dashed] (impl) -- (rtl);
